...
def write_summary_log(success_repos: List[Dict], failed_repos: List[str], log_dir: Path) -> Path:
    """
    Écrit un résumé dans un fichier log dédié.
    
    Args:
        success_repos: Liste de dicts {"name": str, "commit_msg": str}
        failed_repos: Liste de noms de repos en échec
...
def format_notification_message(success_repos: List[Dict], failed_repos: List[str]) -> str:
    """Formate le message pour la notification Windows."""
    lines = []
    
    if success_repos:
        for repo in success_repos[:3]:  # Max 3 repos dans la notif
            commit_short = repo['commit_msg'][:50] + "..." if len(repo['commit_msg']) > 50 else repo['commit_msg']
            lines.append(f"✓ {repo['name']}: {commit_short}")
        
        if len(success_repos) > 3:
            lines.append(f"  +{len(success_repos) - 3} autre(s)")
    
    if failed_repos:
        lines.append(f"❌ {len(failed_repos)} échec(s)")
